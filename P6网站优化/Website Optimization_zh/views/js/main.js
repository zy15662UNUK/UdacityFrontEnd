...
function updatePositions() {
  ...
  // var scrollTopValue = document.body.scrollTop / 1250经过console.log测试始终为0，所以直接去掉
  // phase仅有五种值
  var phase = [];
  for (j = 0; j < 5; j++){
    phase[j] = Math.sin(j);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var moving = document.getElementById("movingPizzas1");
  ...
  for (var i = 0; i < num; i++) {
    ...
    elem.basicLeft = basicLeft[i % 8];
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    moving.appendChild(elem);
  }
  updatePositions();
});
